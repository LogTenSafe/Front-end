...
<script lang="ts">
  ...
  import {
    has, isArray, isNil, isNull, isString
  } from 'lodash-es'
   ...
   * types are a normal HTML `INPUT` or `SELECT`, and the Bootstrap-Vue `b-form-spinbutton` and
   * `b-form-file` inputs.
   */
  ...
  export default class FieldWithErrors extends Vue {
    ...
    get placeholderOrLabel(): string {
      return isNil(this.placeholder) ? this.label : this.placeholder
    }
